...
int main(int, char**)
{
    ...
    while (!glfwWindowShouldClose(engineX->window))
    {
        ...
        for (unsigned int i = 0; i < cube.vertexList.size(); i++)
        {
            ...
            int shiftstate = glfwGetKey(engineX->window, GLFW_KEY_LEFT_SHIFT);
            ...
            int keystate = glfwGetKey(engineX->window, GLFW_KEY_LEFT_CONTROL);
            ...
            for(int ii = 0; ii < selection.size(); ii++){
                ...
                for(unsigned int j = 0; j < 4; j++){
                    ...
                    std::cout << cube.vertexList[ii].model[j][i] << " ";
                    std::cout << std::endl;
                }
            }
            #endif
