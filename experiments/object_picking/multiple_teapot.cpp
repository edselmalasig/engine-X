...
int main(int, char**)
{
     ...
     for(int i = 0; i < 10; i++)
     {
          ...
          /*
          g_teapot[i]->init_cube_wntc();
          g_teapot[i]->init_texture(0, "../../resources/textures/container2.png");
          g_teapot[i]->init_texture(1, "../../resources/textures/container2_specular.png");
          g_teapot[i]->shader->setInt("material.diffuse", 0);
          g_teapot[i]->shader->setInt("material.specular", 1);
          ...
          */
          ...
          g_teapot[i]->model = glm::translate(g_teapot[i]->model, teapotPositions[i]);
          float angle = 20.0f * i;
          g_teapot[i]->model = glm::rotate(g_teapot[i]->model, glm::radians(angle), glm::vec3(1.0f, 0.3f, 0.5f));
          g_teapot[i]->model = glm::scale(g_teapot[i]->model, glm::vec3(0.30f));
     ...
     std::cout << g_teapot[i]->model[3][0] << " " << g_teapot[i]->model[3][1] << " " << g_teapot[i]->model[3][2] << " " << g_teapot[i]->model[3][3] << std::endl;
}
*/
