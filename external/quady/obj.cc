...
static void makeFace(Cell *cell, Tface *f) {
    ...
    for (List_item<Tvert> *vi = f->vlist.first(); vi != 0; vi = vi->next()) {
      ...
      if (vertex1 != 0) {
        // find the next identified vertex, even if just itself
          //std::cout << vertex1->getID() << " " << vertex1->pos[0] << " " << vertex1->pos[1] << " " << vertex1->pos[2] << std::endl;
        ...
        if (vertex2 != 0)
          ...
          //std::cout << vi->obj->vertex->getID() << " " << vi->obj->vertex->pos[0] << " " << vi->obj->vertex->pos[1] << " " << vi->obj->vertex->pos[2] << std::endl;
          ll++;
  ...
  for (;;) {
    vi = vi->next();
      ...
      //i think this is where the edges actually start.
      //std::cout << vertex->getID() << " " << vertex->pos[0] << " " << vertex->pos[1] << " " << vertex->pos[2] << std::endl;
      ecount++;
    if (vi == 0)
      vi = f->vlist.first();
