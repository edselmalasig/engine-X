...
{
  // variables glsl_version, camera, window functions
  // Setup window
  ...
  window = glfwCreateWindow(camera->window_w, camera->window_h, "[engine-X] Very Alpha Stage", NULL, NULL);
...
bool EngineX::solveQuadratic(const float &a, const float &b, const float &c, float &x0, float &x1)
{
...
bool EngineX::RaySphereCollide(glm::vec3 center, float radius, glm::vec3 orig, glm::vec3 dir)
{
...
bool EngineX::RayHitLineSegment(glm::vec3 ray_origin, glm::vec3 ray_direction, Edge e, float coPlanarThreshold, float lengthErrorThreshold)
{
  glm::vec3 da = ray_direction - ray_origin;
  glm::vec3 db = e.ve - e.vs;
  glm::vec3 dc = e.vs - ray_origin;
  #include <math.h>
